#!/bin/bash

mkdir -p ${B2HH_LOG}/TimeErr/out
mkdir -p ${B2HH_LOG}/TimeErr/err
mkdir -p ${B2HH_LOG}/TimeErr/log

mkdir -p ${B2HH_OUT}/TimeErr/templates
mkdir -p ${B2HH_OUT}/TimeErr/test
mkdir -p ${B2HH_OUT}/TimeErr/plots


cd ${B2HH_SRC}/TimeErr
touch *.C
${lbRunDaVinciOld} make 

cd ${B2HH_RUN}/TimeErr
# cuts_bdt="KK_0.04 PIPI_0.12"
# years="201516 2017s29r2p2 2018"
# magnets="Tot"

years=${1//'__'/' '}
magnets=${2//'__'/' '}
cuts_bdt=${3//'__'/' '}
opts=${4//'__'/' '}

rm -f jobs.txt
for cut_bdt in ${cuts_bdt}; do  
    for year in ${years}; do 
        for mag in ${magnets}; do
            for fState in "PIPI" "KK" "KPI"; do
                for opt in ${opts}; do
                    echo ${cut_bdt//"_"/" "} ${year} ${mag} ${fState} ${opt}
                    echo ${cut_bdt//"_"/" "} ${year} ${mag} ${fState} ${opt} >> jobs.txt            
                done
            done
        done
    done
done

condor_submit submit.jdl
