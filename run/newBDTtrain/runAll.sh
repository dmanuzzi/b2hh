...
mkdir -p ${B2HH_LOG}/newBDTtrain/out
mkdir -p ${B2HH_LOG}/newBDTtrain/err
mkdir -p ${B2HH_LOG}/newBDTtrain/log

mkdir -p ${B2HH_OUT}/newBDTtrain/models
mkdir -p ${B2HH_OUT}/newBDTtrain/plots

mkdir -p ${B2HH_OUT}/newBDTtrain/forNorm/
mkdir -p ${B2HH_OUT}/newBDTtrain/forNorm/temp #temporary for the assignment
    ...
    for id in ${ids}; do
        for treeSize in ${treeSizes}; do
            for treeDepth in ${treeDepths}; do
                echo ${model} ${id} ${treeSize} ${treeDepth}
                echo ${model} ${id} ${treeSize} ${treeDepth} >> jobs_train.txt
            done
        done
    done
done
