...
vTag=$3
cutTags=$4
cutTag=$(echo $cutTags | cut -f1 -d-)
CalibImport=$(echo $cutTags | cut -f2 -d-)
cut=$5
importTag=$6
echo $mode $y $vTag $cut
...
cd ../"$mode"_calib
outDir=EPMout_"$y"_"$vTag"_combinedAUTO"$cutTag"
rm -rf $outDir
mkdir $outDir
cd $outDir
optfile=./EPM_options_"$mode"_"$y"_"$vTag""$cutTag"_combinedAUTO.py
...
echo 'PerformOfflineCombination_OS = 1' >> $optfile
echo 'OS_Muon_InOSComb = 1'             >> $optfile
echo 'OS_Electron_InOSComb = 1'         >> $optfile
echo 'OS_Kaon_InOSComb = 1'             >> $optfile
echo 'OS_Charm_InOSComb = 1'            >> $optfile
echo 'VtxCharge_InOSComb = 1'           >> $optfile

echo 'PerformOfflineCombination_SS = 1' >> $optfile
echo 'SS_Pion_InSSComb = 1'             >> $optfile
echo 'SS_Proton_InSSComb = 1'           >> $optfile

#echo 'Selection = "eventNumber % 2 == 1"'  >> $optfile
echo $cut >> $optfile
echo 'import ../EPMout_'$y'_'$vTag$CalibImport'/MyCalibrations.py' >> $optfile

$runEPM $optfile

cp EspressoCalibrations.py MyCalibrations_combinedAUTO.py
