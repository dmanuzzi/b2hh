...
cutTagInput=$(echo $cutTags | cut -f2 -d-)
cut=$5
...
cd ../"$mode"_calib
outDir=EPMout_"$y"_"$vTag"_combined"$cutTag"
rm -rf $outDir
mkdir $outDir
cd $outDir
optfile=./EPM_options_"$mode"_"$y"_"$vTag""$cutTag"_combined.py
cp -f ../../tools/EPM_options_template_"$vTag"_combined.py $optfile
...
if [[ "$y" == "All" ]]; then
	...
	nfin1='"/home/LHCB-T3/dmanuzzi/B2HH/data/'$mode'/'$Y'/MagUp/B2DX_'$Y'_Up_'$mode'_sWeight_wPT_TagComb'$cutTagInput'.root"'
	nfin2='"/home/LHCB-T3/dmanuzzi/B2HH/data/'$mode'/'$Y'/MagDw/B2DX_'$Y'_Dw_'$mode'_sWeight_wPT_TagComb'$cutTagInput'.root"' 
	k=$((k+1))
...
cp EspressoCalibrations.py MyCalibrations_combined.py
