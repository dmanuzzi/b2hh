// this one fixes the compatibility bit of the new trees after the new BDT has been assigned
// also performs a cut on the old BDT value (the one the new BDT is built on)
...
void compatibilityBitFixFinal(TString channel="bspipi", TString year="2015", TString magnet="Up"){

    TString inputPath  = "${B2HH_OUT}/Data/temp/";
    TString outputPath = "${B2HH_OUT}/Data/tuple_merged/"; // main output directory / from this moment the old ones are in the tuple_merged.bak directory
    ...
    //check if it was the copytree or clonetree requirement
    inputTree->CopyTree("bdtPIPI>0")->Write("",TFile::kWriteDelete); // now cutting on old BDT / since we built on top of it
