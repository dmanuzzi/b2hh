...
Int_t main(Int_t argc, Char_t * argv[]) {
  ...
  if(help =="-h" || help == "--help") {
    ...
    printf("Options:\n");
    printf("  -n <name>   : name of the BDT ([PIPI, KK, GraNEW], default = PIPI)\n");
  ...
  Double_t bPT = 0, mass = 0, time = 0, timeErr = 0,
           etaIFT_Bd = 0, etaIFT_Bs = 0, etaDummy = 0;
  ...
  chain->SetBranchAddress("etaIFT_Bd", &etaIFT_Bd);
  chain->SetBranchAddress("etaIFT_Bs", &etaIFT_Bs);
  chain->SetBranchAddress("etaDummy",  &etaDummy);
  ...
  TH1D * h_b2hh_PT     = new TH1D("h_b2hh_PT",    "h_b2hh_PT",50,0,50);
         h_b2hh_PT->Sumw2();
  TH1D * h_bkg_PT      = (TH1D *) h_b2hh_PT->Clone("h_bkg_PT");
  TH1D * h_b2hh_SPD    = new TH1D("h_b2hh_SPD",   "h_b2hh_SPD",20,0,1000);            
         h_b2hh_SPD->Sumw2();
  TH1D * h_bkg_SPD     = (TH1D *) h_b2hh_SPD->Clone("h_bkg_SPD");
  TH2D * h_b2hh_PT_SPD = new TH2D("h_b2hh_PT_SPD","h_b2hh_PT_SPD",50,0,50,20,0,1000); 
         h_b2hh_PT_SPD->Sumw2();
  TH2D * h_bkg_PT_SPD  = (TH2D *) h_b2hh_PT_SPD->Clone("h_bkg_PT_SPD");

  TH1D * h_b2hh_etaIFT_Bd  = new TH1D("h_b2hh_etaIFT_Bd", "h_b2hh_etaIFT_Bd", 100,0,0.5); h_b2hh_etaIFT_Bd->Sumw2();
  TH1D * h_bkg_etaIFT_Bd   = new TH1D("h_bkg_etaIFT_Bd",  "h_bkg_etaIFT_Bd",  100,0,0.5); h_bkg_etaIFT_Bd->Sumw2();
  TH1D * h_b2hh_etaIFT_Bs  = new TH1D("h_b2hh_etaIFT_Bs", "h_b2hh_etaIFT_Bs", 100,0,0.5); h_b2hh_etaIFT_Bs->Sumw2();
  TH1D * h_bkg_etaIFT_Bs   = new TH1D("h_bkg_etaIFT_Bs",  "h_bkg_etaIFT_Bs",  100,0,0.5); h_bkg_etaIFT_Bs->Sumw2();
  TH1D * h_b2hh_etaDummy   = new TH1D("h_b2hh_etaDummy",  "h_b2hh_etaDummy",  100,0,0.5); h_b2hh_etaDummy->Sumw2();
  TH1D * h_bkg_etaDummy    = new TH1D("h_bkg_etaDummy",   "h_bkg_etaDummy",   100,0,0.5); h_bkg_etaDummy->Sumw2();
  ...
  for(Int_t iEntry = 0, nEntries = chain->GetEntries(); iEntry < nEntries; ++iEntry) {
    ...
    if(mass>massMin&&mass<massMax) { 
      ...
      h_b2hh_etaIFT_Bd->Fill(etaIFT_Bd);
      h_b2hh_etaIFT_Bs->Fill(etaIFT_Bs);
      h_b2hh_etaDummy->Fill(etaDummy);
    }
    else if(mass>massMinBkg&&mass<massMaxBkg) {
      h_bkg_PT->Fill(bPT);
      h_bkg_SPD->Fill(nSPDHits);
      h_bkg_PT_SPD->Fill(bPT,nSPDHits);
      h_bkg_etaIFT_Bd->Fill(etaIFT_Bd);
      h_bkg_etaIFT_Bs->Fill(etaIFT_Bs);
      h_bkg_etaDummy->Fill(etaDummy);
    }
  }
  ...
  h_b2hh_etaIFT_Bd->Add(h_bkg_etaIFT_Bd,-myRatio);
  h_b2hh_etaIFT_Bs->Add(h_bkg_etaIFT_Bs,-myRatio);
  h_b2hh_etaDummy->Add(h_bkg_etaDummy,-myRatio);
  ...
  TString outFileName = Form("${B2HH_OUT}/Tagging/iftTemplates/%s_%s_%s_%s_%s.root",name.Data(),bdtCut.Data(),finalState.Data(),year.Data(),magnet.Data());
  printf("updating file: %s",outFileName.Data());
...
//  shiftToZero(h_b2hh_etaSSk);
  ...
  h_b2hh_etaIFT_Bd->Scale(1./h_b2hh_etaIFT_Bd->Integral("width"));
  h_b2hh_etaIFT_Bs->Scale(1./h_b2hh_etaIFT_Bs->Integral("width"));
  h_b2hh_etaDummy->Scale(1./h_b2hh_etaDummy->Integral("width"));

  outFile->WriteTObject(h_b2hh_PT,    Form("h_b2hh_%s%s_PT",    range.Data(),finalState.Data()),"Overwrite");
  outFile->WriteTObject(h_b2hh_SPD,   Form("h_b2hh_%s%s_SPD",   range.Data(),finalState.Data()),"Overwrite");
  outFile->WriteTObject(h_b2hh_PT_SPD,Form("h_b2hh_%s%s_PT_SPD",range.Data(),finalState.Data()),"Overwrite");

  outFile->WriteTObject(h_b2hh_etaIFT_Bd, Form("h_b2hh_%s%s_etaIFT_Bd", range.Data(),finalState.Data()),"Overwrite");
  outFile->WriteTObject(h_b2hh_etaIFT_Bs, Form("h_b2hh_%s%s_etaIFT_Bs", range.Data(),finalState.Data()),"Overwrite");
  outFile->WriteTObject(h_b2hh_etaDummy,  Form("h_b2hh_%s%s_etaDummy",  range.Data(),finalState.Data()),"Overwrite");
