...
def makeDataAsymBsCP(inTree,varNames,varRange,stateName,taggerName,ws, Nbins=7) :
  ...
  h_b = TH1D("h_b","h_b",Nbins,0,T)
  h_bbar = TH1D("h_bbar","h_bbar",Nbins,0,T)

  h_b.Sumw2()
  h_bbar.Sumw2()

  inTree.Draw("fmod(time-%g,%g)>>h_b" % (time.getMin(varRange),T),"weight*(fState==%d&&q%s==1&&p==2&&%s)"%(index,taggerName,dataCut))
  inTree.Draw("fmod(time-%g,%g)>>h_bbar" % (time.getMin(varRange),T),"weight*(fState==%d&&q%s==-1&&p==2&&%s)"%(index,taggerName,dataCut))
