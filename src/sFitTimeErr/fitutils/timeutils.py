...
def createSignalTimePdf(name = 'bdkpi', year = '', config = {}, ws = None) :
  ...
  if 'timeErr' in config['observables'].keys():
    ...
    if config['generate']['flag']:
      timepdfT = ws.obj("%s_pdftimeGenT_%s" % (name,year))
      tmp = WS( ws, RooProdPdf("%s_pdftimeGenTWE_%s" % (name,year),
                               "%s_pdftimeGenTWE_%s" % (name,year),
                               RooArgSet(timeErrPdf),
                               RooFit.Conditional(RooArgSet(timepdfT),listObs)))
      timepdfU = ws.obj("%s_pdftimeGenU_%s" % (name,year))
      tmp = WS( ws, RooProdPdf("%s_pdftimeGenUWE_%s" % (name,year),
                               "%s_pdftimeGenUWE_%s" % (name,year),
                               RooArgSet(timeErrPdf),
                               RooFit.Conditional(RooArgSet(timepdfU),listObs)))
