...
for year in modelYears:
  ...
  for name in [ 'bdkpi','bskpi', 'bskk' ]:
    ...
    nfinTimeErrSignal = inputs['timeErr']['file'].format(fState  = selConf[name]['state'],
                                                         bdtName = selConf['bdt']['name'],
                                                         bdtCut  = selConf['bdt']['cut'],
                                                         year    = year,
                                                         magnet  = selConf['magnet'])
    ...
    getTemplates(name,year,nfinTimeErrSignal,
                 [ 'timeErr' ], ws)
    ...
    pdftime = ws.obj("%s_pdftimeWE_%s" % (name,year))
...
for year in args.years:
  ...
  for name in ['bskk']:
    ...
    print('STOCAZZO')
    integralPdfT.Print("v")
    integralPdfU.Print("v")
