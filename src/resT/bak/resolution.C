...
using namespace std;
int resolution(TString tag, vector<TString> nfins){
  

  gROOT->SetStyle("Plain");
  
  Bool_t isMC   = tag.Contains("MC");
  Bool_t isJpsi = tag.Contains("Jpsi");
  Bool_t isY1S  = tag.Contains("Y1S");
  Bool_t isBs2KK= tag.Contains("bskk");
  Bool_t doPIDsel=tag.Contains("PIDsel");
  ...
  else if (isBs2KK){ Mmin = 5.00, Mmax= 6.2; }
  ...
  if (isJpsi || isY1S){
    ...
    if (tag.Contains("sw")){
      inChain->SetBranchStatus("Nsig_sw", 1);
      inChain->SetBranchAddress("Nsig_sw",    &Nsig_sw) ;
    }
    if (isMC){
  ...
  Double_t piminusDLLKPI=0., piminusDLLPPI=0.;
  Int_t fromBc=-1;
  if (isBs2KK){
    inChain->SetBranchStatus("tauKK"   , 1);
    inChain->SetBranchStatus("tauKKErr", 1);
    inChain->SetBranchStatus("massKK"  , 1);
    inChain->SetBranchStatus("bPT"     , 1);
    inChain->SetBranchStatus("bdtKK"   , 1);
    inChain->SetBranchStatus("piplusDLLKPI" ,  1);
    inChain->SetBranchStatus("piplusDLLPPI" ,  1);
    inChain->SetBranchStatus("piminusDLLKPI",  1);
    inChain->SetBranchStatus("piminusDLLPPI",  1);
    
    inChain->SetBranchAddress("tauKK"   , &tau);
    inChain->SetBranchAddress("tauKKErr", &tauErr);
    inChain->SetBranchAddress("massKK"  , &head_M);
    inChain->SetBranchAddress("bPT"     , &head_PT);
    inChain->SetBranchAddress("bdtKK"   , &bdtKK);
  ...
  TH1D * hTauE  = new TH1D("TauE_"+tag, "TauE_"+tag+"; RMS_{#tau} [ps]",      100, minVal, maxVal);

  TFile * outFile = new TFile("./tuples/"+tag+".root","RECREATE");
  ...
  Double_t out_bdtKK=0., out_piplusDLLKPI=0., out_piplusDLLPPI=0.,  out_piminusDLLKPI=0., out_piminusDLLPPI=0.;
  Int_t out_fromBc=0;
  if (isMC && isBs2KK){
    outTree->Branch("bdt", &out_bdtKK);
    outTree->Branch("piplusDLLKPI", &out_piplusDLLKPI, "piplusDLLKPI/D");
    outTree->Branch("piplusDLLPPI", &out_piplusDLLPPI, "piplusDLLPPI/D");
    outTree->Branch("piminusDLLKPI", &out_piminusDLLKPI, "piminusDLLKPI/D");
    outTree->Branch("piminusDLLPPI", &out_piminusDLLPPI, "piminusDLLPPI/D");
    outTree->Branch("fromBc", &out_fromBc, "fromBc/I");
  }
  ...
  for(Int_t iEntry = 0; iEntry < nEntriesIN; iEntry++) {
    ...
    if (isBs2KK){
      if (bdtKK < 0.04)    continue;
      if (isMC && fromBc!=0) continue;
      if (doPIDsel && piplusDLLKPI  < 2)               continue;
      if (doPIDsel && piplusDLLKPI-piplusDLLPPI  < -2) continue;
      if (doPIDsel && piminusDLLKPI < 2)               continue;
      if (doPIDsel && piminusDLLKPI-piplusDLLPPI < -2) continue;
      out_bdtKK = bdtKK;
      out_piplusDLLPPI = piplusDLLPPI;
      out_piplusDLLKPI = piplusDLLKPI;
      out_piminusDLLPPI= piminusDLLPPI;
      out_piminusDLLKPI= piminusDLLKPI;
      out_fromBc = fromBc;
    }
  ...
  meanTot /= totTot;
  FILE* fStats = fopen("stats/stats"+tag+".txt", "a");
  fprintf(fStats, "********* %s ********* \n", tag.Data());
  ...
  cSF->SaveAs("plots/"+tag+".png");
  cSF->SaveAs("plots/"+tag+".pdf");

  TCanvas *cM = new TCanvas("cM_"+tag, "cM_"+tag);
  hJpsiM->SetMarkerStyle(20);
  hJpsiM->SetMarkerColor(kBlack);
  hJpsiM->Draw("PE1");
  cM->SaveAs("plots/M_"+tag+".png");
  cM->SaveAs("plots/M_"+tag+".pdf");

  TCanvas *cTau = new TCanvas("cTau_"+tag, "cTau_"+tag);
  hTau->Draw("hist");
  cTau->SaveAs("plots/tau_"+tag+".png");
  cTau->SaveAs("plots/tau_"+tag+".pdf");
  TCanvas *cTauE = new TCanvas("cTauE_"+tag, "cTauE_"+tag);
  hTauE->Draw("hist");
  cTauE->SaveAs("plots/tauE_"+tag+".png");
  cTauE->SaveAs("plots/tauE_"+tag+".pdf");
  ...
  fit2Box->Draw("same");  
  cTauMean->SaveAs("plots/tauMean_"+tag+".png");
  cTauMean->SaveAs("plots/tauMean_"+tag+".pdf");
