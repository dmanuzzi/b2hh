...
Int_t main(Int_t argc, Char_t * argv[]){
  ...
  if(help =="-h" || help == "--help") {
    ...
    printf("  -w <weightName> : 'NONE' to avoid weights, default = sWeight\n");
  ...
  bool isBs2KK    = decay.Contains("bskk");
  bool isBs2pipi  = decay.Contains("bspipi");
  
  TString tag_tuple = Form("%s%s_%s%s_%s_%s",
			   dataType.Data(),(isWeighted?"w":""),
			   decay.Data(),
			   (bdtName=="NONE")?"":Form("_%s_%s",bdtName.Data(),bdtCut.Data()),
			   year.Data(),magnet.Data());
  TString tag = tag_tuple+"_"+fitModel;
...
*/
  ...
  TChain * chain = new TChain("chain_"+tag,"chain_"+tag);
  chain->Add("${B2HH_OUT}/resT/tuples/"+tag_tuple+".root/ntp");
  chain->Print();
  ...
  if (isMC){
    if (isB2HH) pdf = new RooProdPdf("pdf","pdf",*errPdf,Conditional(*g12,*deltaTau));
  ...
  TString nfin_params = "${B2HH_OUT}/resT/params/paramsRes_"+tag+".txt";
  std::cout<< "nfin_params = " << nfin_params.Data() << "\n";
  expandFileName::expandFileName(nfin_params);
  params->readFromFile(nfin_params);
  ...
  r1 = pdf->fitTo(*dataFinal, Offset(),  Verbose(kTRUE), PrintLevel(3), Strategy(2), Save(), NumCPU(8), SumW2Error(!isMC));
  ...
  // }
  //return 0;
  params->writeToFile(nfin_params);
  ...
  TFile * outFile = new TFile("${B2HH_OUT}/resT/plots/resFit_"+tag+".root","RECREATE");
  ...
  while (rangeMax<timeErrMax+0.001){
    ...
    lowerPad[bin]->cd();
    pulls[bin]->Draw("B");
    c[bin]->SaveAs(Form("${B2HH_OUT}/resT/plots/resFit_%s_bin%d.pdf", tag.Data(), bin));
  ...
  if (isMC && isB2HH) pdf->plotOn(plotTot,Components(*g2),LineColor(kCyan+2),LineStyle(kDashed));    
  //  if (!isMC){
  ...
  cTot->SaveAs("${B2HH_OUT}/resT/plots/resFit_"+tag+".pdf");
  upperPadTot->SetLogy();
  cTot->SaveAs("${B2HH_OUT}/resT/plots/resFit_"+tag+"_log.pdf");
  outFile->ls();
  outFile->Close();
