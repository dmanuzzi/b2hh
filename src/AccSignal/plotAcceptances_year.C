...
int main(int argc, char *argv[]){
  ...
  TFile fout(Form("${B2HH_OUT}/AccSignal/plots/%s.root", config.Data()), "RECREATE");
  ...
  for (auto it : year_color){
    ...
    TString tmp_nfin = Form("${B2HH_OUT}/AccSignal/plots/plot%s_%s_%s_%s_%s_%s_%s.root", 
    			    tag.Data(), mode.Data(),
    			    bdtType.Data(), bdtCut.Data(), 
    			    tmp_year.Data(), magnet.Data(), suffix.Data());
  ...
  //leg->SetY2(0.75);
  leg->Draw("same");
  c.SaveAs(Form("${B2HH_OUT}/AccSignal/plots/%s.root", config.Data()));
  //  c.SaveAs(Form("plots/%s.png" , config.Data()));
  c.SaveAs(Form("${B2HH_OUT}/AccSignal/plots/%s.pdf" , config.Data()));
