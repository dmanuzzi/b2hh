...
int main(int argc, char * argv[]) {
  ...
  TH2D * hPT_SPD;
  finalState.ToLower();
  TFile *outFile = TFile::Open(Form("${B2HH_OUT}/TimeErr/templates/%s_%s_%g_%s_%s.root",
  ...
  for(auto channel: channels) {
    ...
    hTimeErr = new TH1D(Form("h_%s_timeErr",channel.Data()),Form("h_%s_timeErr",channel.Data()),
                        70,0,0.1);
    ...
    for(Int_t iEntry = 0, nEntries = chain->GetEntries(); iEntry < nEntries; ++iEntry) {
      ...
      hTimeErr->Fill(timeErr,weight*weightPID);
    ...
    hTimeErr->Sumw2();
    hTimeErr->Smooth(1,"r");
    hTimeErr->Scale(1./hTimeErr->Integral("width"));
    hPT->Sumw2();
    ...
    shiftToZero(hTimeErr);
    outFile->WriteTObject(hTimeErr,"","Overwrite");
    outFile->WriteTObject(hPT,"","Overwrite");
    outFile->WriteTObject(hSPD,"","Overwrite");
    outFile->WriteTObject(hPT_SPD,"","Overwrite");
    delete hTimeErr; delete hPT; delete hSPD; delete hPT_SPD;
