...
int main(int argc, char * argv[]){
  ...
  if (mode == "CAL"){
    ...
    outFile = new TFile("${B2HH_OUT}/PID/effB2HH/plots/particleKinematic_CAL_"+tag+".root","RECREATE");
    TFileCollection * calList = new TFileCollection("list_"+tag,"list"+tag,"${B2HH_OUT}/PID/prepareCalib/PIDcalibLists/list_"+tag+".txt");
    ...
    outFile = new TFile("${B2HH_OUT}/PID/effB2HH/plots/particleKinematic_SIG_"+tag+".root","RECREATE");
    ...
    chainAdder::chainAdder(sigChain, "${B2HH_OUT}/Data/tuple_merged/%s_%s_%s.root/b2hh",
			   MCsamples[particle+charge].Data(), tag_years, tag_magnet);
    sigChain->Print();
    hKin = new TH2D("hKin_"+tag,"",250,0,500,20,1.,6.);
  ...
  c->SaveAs("${B2HH_OUT}/PID/effB2HH/plots/particleKinematic_"+mode+"_"+tag+".pdf");
  c->SaveAs("${B2HH_OUT}/PID/effB2HH/plots/particleKinematic_"+mode+"_"+tag+".png");
