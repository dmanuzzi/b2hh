...
elif 'SameVeloHalf' in args.outDir:
  chainNew = chain.CopyTree("(piplusPx*piminusPx>0)")
elif 'DiffVeloHalf' in args.outDir:
  chainNew = chain.CopyTree("(piplusPx*piminusPx<0)")
elif 'lowLogIP2overIPCHI2' in args.outDir:
  chainNew = chain.CopyTree("log(bIP*bIP/bIPCHI2)<-8.4")
elif 'highLogIP2overIPCHI2' in args.outDir:
  chainNew = chain.CopyTree("log(bIP*bIP/bIPCHI2)>-8.4")
elif 'lowPT' in args.outDir:
  chainNew = chain.CopyTree("bPT<7")
elif 'highPT' in args.outDir:
  chainNew = chain.CopyTree("bPT>7")
elif 'lowP' in args.outDir:
  chainNew = chain.CopyTree("bP<87")
elif 'highP' in args.outDir:
  chainNew = chain.CopyTree("bP>87")
else:
  chainNew = chain
chainNew.Print()
...
for i in range(data.numEntries()):
  ...
  if 'noDTF' in args.outDir:
    ...
    m = (2*(mk2+E12-P12))**0.5
    tmpTimeOld = m/p*l/c
    if 'noDTF_nominalMass' in args.outDir:
      tmpTimeOld = 5.3663/p*l/c
