...
def createBkgTimePdf(name = 'bkg_kpi', year = '', config = {}, selConf = {}, sstagName = "SS", ws = None) :
  ...
  if 'template' in confTime.keys():
    ...
    templateFile = TFile('../TimeModels/templatesHigh_%s_%s_%s_%s_%s.root'%(finalState,
                                                                            selConf['bdt']['name'],
                                                                            selConf['bdt']['cut'],
                                                                            year,selConf['magnet']),'read')
    ...
    #pdf10 = WS(ws, myWS.pdf('%s_pdftime10_%s' % (name,'Tot')))
    #pdf01 = WS(ws, myWS.pdf('%s_pdftime01_%s' % (name,'Tot')))
...
def createPhysTimePdf(name = 'bkg_kpi', year = '', config = {}, selConf = {}, sstagName = "SS", ws = None) :
  ...
  if 'template' in confTime.keys():
    ...
    templateFile = None
    templateFile = TFile('../TimeModels/templatesPhys_%s_%s_%s_%s_%s.root'%(finalState,
                                                              selConf['bdt']['name'],
                                                              selConf['bdt']['cut'],
                                                              year,selConf['magnet']),'read')
    templateFile.Print("v")
