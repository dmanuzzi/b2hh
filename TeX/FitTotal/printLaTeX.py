...
dirs = ['GraNEW_0.86_201516_Tot', 'GraNEW_0.86_2017s29r2p2_Tot', 'GraNEW_0.86_2018_Tot']
...
def printResultsTable(fout, results, varsets, tags, label, caption):
    ...
    fout.write('\\centering\n')
    #fout.write('\\begin{adjustbox}{max width=\\textwidth}\n')
    ...
    #fout.write('}\n')  #closing resize/adjustboxbox
    fout.write('\\caption{%s}\n'%caption)
    fout.write('\\label{%s}\n'%label)
    fout.write('\\end{table}\n')


def printResultsTableResized(fout, results, varsets, tags, label, caption):
    values = {}
    ntags = []
    requirements = []
    for varset in varsets:
...
for year,mag in product(years,mags):
        ...
        for sel,bdt in configs:
            ...
            for fState in fStates:
                ...
                for slice in slices:
                    if slice == 'sig5.45' and fState == 'KPI': continue
                    ...
                    for var in vars:
                        ...
                        width = 0.3  if var == 'asym' else 0.19
                        figList += '.'
...
for config in configs:
    ...
    for varsets in varsetss:
        ...
        Variable set: %s'''%(config[0],config[1], ', '.join(varsets))
        printResultsTableResized(fout, results, varsets = varsets,
                          tags = tags, label=label, caption=caption)
