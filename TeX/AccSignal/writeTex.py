...
years = ['Tot','201516', '2017s29r2p2', '2018']
years = ['Tot']
...
tags  = ['tot', 'tag']
taggers = ['OS_SSk','IFT_Bs_Dummy','IFT_Bd_Dummy']
...
fout.write('\\usepackage{placeins}\n')
fout.write('\\newcommand{\\code}[1]{\\texttt{\\detokenize{#1}}}\n')
#fout.write('\\usepackage{grffile}\n')
fout.write('\\graphicspath{{/ceph-data/lhcb_g/users/mcaporal/ift_b2hh_run2/out/AccSignal/}}\n') #new
...
for year in years:
    ...
    for conf in confs:
        ...
        for fstate, modes in iter(modess.items()):
            for tagPair in taggers:
                #if not tagPair == taggers[0]:
                #    fout.write('\\clearpage\n')
                #fout.write('\\paragraph{Tagger pair: \\texttt{%s}}\n' % tagPair.replace('_',' '))
                fout.write('\\FloatBarrier\n')
                nfigs=[]
                for tag in tags:
                    for mode in modes:
                        nfigs+= ['plots/fitAcc/pdf/fitForAcc_%s_%s_%s_%s_Tot_%s.pdf'%(mode,conf,tagPair,year,tag)] # {} removed as they were showing issues
                addFigure(fout, nfigs,
                      label='fitForAcc %s %s %s'%(year, bdtType, tagPair),
                      caption='Fit for acceptaces. Year %s. Final State: %s. Selection: $\\mathtt{bdt%s}>%s$. TaggerPair: \\texttt{%s}. Top: \\texttt{untagged}, bottom: \\texttt{tagged}.'%(year,fstate, bdtType, bdtCut, tagPair.replace('_',' ')),
