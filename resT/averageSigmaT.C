...
int averageSigmaT(TString year = "2018"){
  ...
  if (year == "2015" || year == "2016" || year == "201516"){
    ...
    nfins = {"${B2HH_HOME}/out/Reduce/b2hh_KK_0.1_201516_Tot_moreVaraibles.root/b2hh"};
    ...
    nfins = {"${B2HH_HOME}/out/Reduce/b2hh_KK_0.1_2017s29r2p2_Tot_moreVaraibles.root/b2hh"};
  } else if (year == "2018"){
    ...
    nfins = {"${B2HH_HOME}/out/Reduce/b2hh_KK_0.1_2018_Tot_moreVaraibles.root/b2hh"};
  } else return 0;
  ...
  Double_t deltaM = (isMC?17.8:17.765);
  TChain c("ntp");
  ...
  UInt_t runNumber=0;
      c.SetBranchStatus("*", 0);
  c.SetBranchStatus("tauKK", 1);
  c.SetBranchAddress("tauKK", &tauKK);
  ...
  if (!isMC){
    c.SetBranchStatus("massKK",1);
    c.SetBranchAddress("massKK", &mass);
  } else {
  ...
  for (Int_t i=0;i<nentries; ++i){
    c.GetEntry(i);
    if (!isMC && (mass<5.33 || mass>5.45) )         continue;
    // if (tauKK<2) continue;
    if (runNumber>214390) continue;
