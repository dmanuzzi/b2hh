...
TH1D* plotAsymFast(TString nfin,  TString name, Int_t nBins = 7, 
  ...
  tin->Draw("fmod(tauKK-"+minT+",0.35498222)>>hB_"+name, "weight*(qOS==1 && tauKK>"+minT+" && "+bCut+")", "goff");
  tin->Draw("fmod(tauKK-"+minT+",0.35498222)>>hBbar_"+name,"weight*(qOS==-1 && tauKK>"+minT+" && "+bCut+")", "goff");
  hSum->Add(hB);
  hSum->Add(hBbar);
  hDif->Add(hBbar);
  hDif->Add(hB, -1);
